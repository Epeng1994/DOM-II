...
logoHeader.addEventListener('mouseover', event =>{
    event.target.textContent = 'Hop on in kids'
})
...
navA.forEach(a => a.addEventListener("click", event => {
    logoHeader.focus()
    logoHeader.textContent = 'Over Here'
}))
...
box.value = 'Select text here to display'
...
press.textContent = 'Click Me'
introP.appendChild(press)
press.addEventListener('click', e=>{
    const home = document.querySelector('.home')
    home.style.backgroundColor = 'rgb(155, 102, 102)'
    home.style.transition = '.5s'
})

const revert = document.createElement('button')
revert.textContent = 'Return'
